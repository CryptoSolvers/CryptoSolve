...
\begin{itemize}
	...
	all of the current cipher blocks it knows. \todo{Brandon: Not sure how I feel about this wording. The advesary only needs to unify the two blocks within $E_j$ each time it receives it. Since this algorithm non-deterministically chooses a solution it does not matter, but otherwise it could try to unify the top block in one sequence with a bottom block in another.}
	\todo{It's a requirement, we can't limit what blocks the
		adversary attempts to unify. However, we can use nonces
		to ensure only certain pairs could possibly be unifiable.}
...
\todo{Andrew: Could do this by using multiple sessions or
	...
	What do you think?}
\todo{Brandon: I like the first option better as that allows us to have a bounded number of sessions. (The undeterministic proof currently only requires one session) }
...
\section{Bounded Session Lengths and Unbounded Number of Sessions}
\thispagestyle{fancy}
...
x_2 & \qquad y_2 \, := \, \overline{\alpha_i}\big( x_2 \; \oplus \;   f(x_1)  \big) \; ~ \oplus ~ \; f( y_1 ) \\[+10pt]
x_3 & \qquad y_3 \, := \,  \overline{\beta_i}\big( x_3 \; \oplus \; f(f(x_1)) \big) \; ~ \oplus ~ \; f(f( y_1 )) \\[+10pt]
\end{array}
\]
...
\begin{array}{cl}
x_1 & \qquad y_1 \, := \, r \; \oplus \; f( x_1 ) \\[+10pt]
x_2 & \qquad y_2 \, := \, \overline{\alpha_n}( x_2 ) \; \oplus \;   f( y_1 ) \\[+10pt]
x_3 & \qquad y_3 \, := \,  \overline{\beta_n}( x_3 ) \; \oplus \; f(f( y_1 )) \\[+10pt]
\end{array}
\]
...
\begin{array}{cl}
x_1 & \qquad y_1 \, := \, r \; \oplus \; f( x_1 ) \\[+10pt]
x_2 & \qquad y_2 \, := \, \overline{\alpha_0}\big( x_2  \; \oplus \;   f(x_1) \big)\\[+10pt]
x_3 & \qquad y_3 \, := \,  \overline{\beta_0}\big( x_3  \; \oplus \; f(f(x_1)) \big)\\[+10pt]
\end{array}
\]
