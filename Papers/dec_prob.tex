...
In this section we look at another possible encoding, where the
cipher blocks contain pairs. The encoding is done as follows:
\begin{itemize}
	...
	\item Each cipher block, $C_i$, is the encoding of a pair
	$[A_i, B_i]$, where $A_i$ is a cipher block $f$-encoding of upper strings of the PCP, and $B_i$ the $f$-encoding of lower strings.
	...
	\end{itemize}  
	\item For each pair in a cipher block we distinguish the first of the second element of the pair by a subscript. For example, if $C_i = [t_1, t_2]$, then 
	$C_{i,1} = t_1$ and $C_{i,2} = t_2$.
