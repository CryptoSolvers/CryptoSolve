...
\section{$MOE_{\oplus}$ Decision Problems Approach One}\todo[inline]{Andrew: approach that is perhaps easier}
...
\subsection{The $MOE_{\oplus}$-decision problem for Arbitrary Sessions of Arbitrary Length}
Let's now consider the $MOE_{\oplus}$-decision problem
where we assume unbounded number of sessions and unbounded session lengths.
...
\section{$MOE_{\oplus}$ Decision Problems Approach Two}
We give a second approach to the proofs in this section. For this approach to work we need to make the following assumptions:
\begin{enumerate}
...
\subsubsection{Chaining multiple strings:}
...
\subsubsection{Adding/Removing a final nonce to encode the index:}
