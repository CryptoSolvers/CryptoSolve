...
need to be able to check that this property exists. For cryptosystems
modeled symbolically as MOEs this leads to the invertability problem.
...
Then, the \emph{invertiablity problem} asks whether there is an 
algorithm which when given $M$ and $C_i$ will produce $m$. 
\todo{Brandon: If it took multiple cipher text blocks to encode $m$, then just having one $C_i$ wouldn't be enough to produce $m$ wouldn't it?}
\end{definition}
