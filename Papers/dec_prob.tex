...
\subsection{Encoding Bounded Session length Encoding}
...
$\sigma$ is a substitution, s.t., 
$\sigma = \{x_0 \mapsto C_0, x_1 \mapsto C_1, \ldots, 
	x_n \mapsto C_n\}$. Thus, $\sigma$ represents the cipher blocks, $C_i$, produced by the $MOE$ at step 
...
problem is computationally equivalent to the intruder deduction problem. 
\end{lemma}
...
Here we limit our investigation to signature 
$\Sigma = \{\oplus, 0, f, f^{-1}\}$ and 
$MOE_{\oplus}$ modes of encryption. 

The equational theories can be presented as a combination.
$R_{f} = \{ f(f^{-1}(M)) \rightarrow M, ~f^{-1}(f(M)) \rightarrow M \}$.  $R_{\oplus} = \{x \oplus x \rightarrow 0, ~x \oplus 0 \rightarrow x\}$ and 
$E_{\oplus} = AC(\oplus)$. We are interested in the 
invertibility problem for the theory 
$R_{f} \cup R_{\oplus} \cup E_{\oplus}$.

First consider the deduction problem for 
$R_{\oplus} \cup E_{\oplus}$ this problem has been 
solved in the positive in~\cite{DBLP:journals/tcs/AbadiC06}.
Furthermore, the theory $R_{f}$ is subterm convergent and thus the duduction problem is also solvable for it 
due to~\cite{Abadi2006}. Therefore we have the following.

\begin{lemma}\label{lem:deduc_xor_dec}
(\cite{Abadi2006})\\	
The deduction problem is solvable in both $R_{\oplus} \cup E_{\oplus}$ and $R_{f}$
\end{lemma}
\begin{proof}~\\
\begin{itemize}
	\item $R_{\oplus} \cup E_{\oplus}$ is proven in
	~\cite{Abadi2006}.
	\item $R_{f}$ follows from the fact that $R_{f}$ is
	subterm convergent and the deduction result on 
	subterm-convergent theories developed in~\cite{Abadi2006}.
\end{itemize}
\end{proof}

Thus, we can consider the combination problem for 
the theory  $R_{f} \cup \{R_{\oplus} \cup E_{\oplus} \}$.
Notice that this is a disjoint combination since 
$\{f, f^{-1}\} \cap \{0, \oplus\} = \emptyset$. 
Therefore we get the following.

\begin{lemma}\label{lem:ded_full}
The deduction problem is solvable in the theory 
$R_{f} \cup \{R_{\oplus} \cup E_{\oplus} \}$.
\end{lemma}
\begin{proof}
Follows from the disjoint combination result 
of~\cite{Cortier2010}.
\end{proof}

Therefore, combining Lemma~\ref{lem:ded_full} with 
Lemma~\ref{lem:invert_equiv_deduc} we get the 
following.

\begin{corol}\label{cor:xor_inver_decidable}
The invertibuility problem for the theory 
$R_{f} \cup \{R_{\oplus} \cup E_{\oplus} \}$ is
decidable.
\end{corol}

\subsubsection{Dedicated $MOE_{\oplus}$ Invertibility  Algorithm}\todo{Andrew: Develop a dedicated algorithm building on Brandon's  ideas and show it's more efficient than
	the combination method.}
While we know that an algorithm exists for solving the 
invirtibility problem for the theory $R_{f} \cup \{R_{\oplus} \cup E_{\oplus} \}$ we looked to 
develop a dedicated algorithm for this theory that could
be easily implemented into an invitibility checking 
tool and improve upon the efficiency of the generic 
combination algorithms. We present that algorithm here.
