...
from moe.moo import cipher_block_chaining, propogating_cbc, hash_cbc, cipher_feedback, output_feedback
...
chaining = dict(
    ...
    OutputFeedback=output_feedback
)

def valid_unif_chaining(unif_choice, chaining_moe):
    """
    Responds true if the unification algorithm chosen
    and the chaining method chosen are not compatible.
    """
    if unif_choice != p_syntactic and unif_choice != XOR_rooted_security:
        return True
    if unif_choice == p_syntactic:
    ...
    return chaining_moe in supported_chaining
...
def index():
    if request.method == 'POST':
        ...
        if not valid_unif_chaining(unif_choice, chaining_moe):
            response = "INVALID UNIFICATION AND CHAINING COMBO"
        else:
            ...
            result = MOE(unif_choice, chaining_moe, schedule, length_bound, 1, knows_iv) if unif_choice is not None and chaining_moe is not None else "TRY AGAIN"
